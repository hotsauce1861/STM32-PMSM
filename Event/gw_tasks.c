...
void task_init(void){
	...
	*/
	...
	foc_motor_start(&foc_obj, 15000, Delay);
	foc_obj.feedback.sector = get_pos_rotor_2();
	encoder_set_to_zero_position();
	/*
		software init		
	*/
	...
	foc_obj.speed_pi.hKpGain = 10;
	foc_obj.speed_pi.hKiGain = 0;
	foc_obj.speed_pi.hKdGain = 0;	
	foc_obj.speed_pi.hLowerOutputLimit = -100;
	foc_obj.speed_pi.hUpperOutputLimit = 100;
	
	PID_HandleInit(&foc_obj.cur_d_pi);
	foc_obj.cur_d_pi.hKpGain = 1;
	foc_obj.cur_d_pi.hKpDivisor = 2;
	foc_obj.cur_d_pi.hKiGain = 1;
	foc_obj.cur_d_pi.hKdGain = 2;	
...
void task_svpwm(void){
	...
	volt_out = park_rev(foc_obj.vol_dq, i+=128);
	...
	uart_data[2] = -foc_obj.feedback.ia*10 - foc_obj.feedback.ib*10;
	uart_data[3] = foc_obj.feedback.theta;
	//printf("%d,\n",Ia);
...
void task_svpwm1(void){
	...
	#if USE_SPEED_PID
	pi_speed_out = PI_Controller(&foc_obj.speed_pi,speed_set - foc_obj.feedback.rpm);
	#endif	
	pi_speed_out = -25000;
	iq_set = pi_speed_out;
	id_set = 0;
	...
	uart_data[0] = pi_id_out;
	uart_data[1] = pi_iq_out;
	uart_data[2] = foc_obj.feedback.theta * PN;
	uart_data[3] = foc_obj.feedback.sector * 1000;	
